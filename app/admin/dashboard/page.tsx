...
export default function AdminDashboard() {
  ...
  const [productForm, setProductForm] = useState({
    ...
    price: "",
    original_price: "",
    regular_price: "", // تمت إضافة السعر العادي
    installation_price: "", // تمت إضافة سعر التركيب
    category: "",
    brand: "",
    image: "",
    local_image_path: "",
  ...
  const handleProductSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const productData = {
        ...productForm,
        price: Number.parseFloat(productForm.price),
        original_price: productForm.original_price ? Number.parseFloat(productForm.original_price) : 0,
        regular_price: productForm.regular_price ? Number.parseFloat(productForm.regular_price) : 0, // تمت الإضافة
        installation_price: productForm.installation_price ? Number.parseFloat(productForm.installation_price) : 0, // تمت الإضافة
        features: productForm.features.split("\n").filter((f) => f.trim()),
        rating: Number.parseFloat(productForm.rating),
        reviews: Number.parseInt(productForm.reviews),
        specifications: {},
  ...
  const resetProductForm = () => {
    setProductForm({
      ...
      price: "",
      original_price: "",
      regular_price: "", // تمت الإضافة
      installation_price: "", // تمت الإضافة
      category: "",
      brand: "",
      image: "",
      local_image_path: "",
  ...
  const editProduct = (product: Product) => {
    ...
    setProductForm({
      ...
      price: product.price.toString(),
      original_price: product.original_price?.toString() || "",
      regular_price: product.regular_price?.toString() || "", // تمت الإضافة
      installation_price: product.installation_price?.toString() || "", // تمت الإضافة
      category: product.category,
      brand: product.brand,
      image: product.image || "",
      local_image_path: product.local_image_path || "",
                  ...
                  <DialogContent className="max-w-2xl bg-white/95 backdrop-blur-sm max-h-[90vh] overflow-y-auto">
                    ...
                    <form onSubmit={handleProductSubmit} className="space-y-4">
                      ...
                      <div className="grid md:grid-cols-4 gap-4">
                        ...
                        <div>
                          <Label htmlFor="product-regular-price" className="text-easyoft-navy">
                            السعر 
                          </Label>
                          <Input
                        ...
                        <div>
                          <Label htmlFor="product-installation-price" className="text-easyoft-navy">
                            سعر التركيب
                          </Label>
                          <Input
                            id="product-installation-price"
                            type="number"
                            step="0.01"
                            value={productForm.installation_price}
                            onChange={(e) => setProductForm({ ...productForm, installation_price: e.target.value })}
                            className="border-easyoft-sky focus:border-brand-primary"
                          />
                        </div>
                        <div>
                        ...
                        <div className="grid grid-cols-2 gap-2 mb-4 text-sm">
                          {product.regular_price && (
                            <div className="bg-gray-100 p-2 rounded">
                              ...
                              <div className="font-medium">{product.regular_price} ر.س</div>
                            </div>
                          )}
                          {product.installation_price && (
                            <div className="bg-blue-50 p-2 rounded">
                              <div className="text-blue-600">سعر التركيب</div>
                              <div className="font-medium text-blue-700">{product.installation_price} ر.س</div>
                            </div>
                          )}
                        </div>
